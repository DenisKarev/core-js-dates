...
function getNextFriday(date) {
  const nextFriday = new Date(date.setUTCDate(date.getUTCDate() + 1));
...
function isDateInPeriod(date, period) {
  ...
  const dateItself = new Date(date);
  return dateItself >= startDate && dateItself <= endDate;
}
...
function formatDate(date) {
  ...
  const dateItself = new Date(date);
  return dateItself.toLocaleDateString('en-US', options);
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= days; i += 1) {
    const currentDay = new Date(year, monthFromZero, i);
...
function getWeekNumberByDate(date) {
  ...
  const yearBegin = new Date(Date.UTC(copy.getUTCFullYear(), 0, 1));
  const weekNumber = Math.ceil(((copy - yearBegin) / dayInMillis + 1) / 7);
  return weekNumber;
}
